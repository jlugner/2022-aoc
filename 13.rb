...
def compair(packet1, packet2)
  case [packet1, packet2]
...
# A
pairs = input.split("\n\n")
pairs.map! { |package| package.lines.map { YAML.load(_1) } } # Parse from string to array

valid_indices = []
pairs.each_with_index do |pair, index|
  pair.each_slice(2) do |packet1, packet2|
    valid_indices << index + 1 if compair(packet1, packet2).positive?
  end
end

pp "a: #{valid_indices.sum}"

# B
packets = input.lines.map { YAML.load(_1) }.compact
...
packets = packets.sort { compair(_2, _1) }
