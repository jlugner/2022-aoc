rucksaks = DATA.readlines
PRIORITIES = ('a'..'z').to_a + ('A'..'Z').to_a

# A
sum_of_mistake_priorities = rucksaks.map do |rucksack|
  section1, section2 = rucksack.chars.each_slice(rucksack.size / 2).map(&:join)
  mistake = (section1.chars & section2.chars).first
  PRIORITIES.index(mistake) + 1
end.flatten.sum

print "a: #{sum_of_mistake_priorities} \n"

# B
sum_of_badge_priorities = rucksaks.map(&:chars).each_slice(3).map do |rucksack1, rucksack2, rucksack3|
  badge = (rucksack1 & rucksack2 & rucksack3).first
  PRIORITIES.index(badge) + 1
end.sum

print "b: #{sum_of_badge_priorities} \n"
__END__
JppMDcJPcQbqGqFb
